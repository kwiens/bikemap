  ...
  const handleLayerToggle = useCallback(async (event: CustomEvent) => {
    ...
    if (layer === 'bikeRentals') {
      ...
      if (visible && map.current) {
        ...
        bikeRentalMarkers.current.forEach((marker: mapboxgl.Marker) => {
          marker.remove();
        });
        ...
        try {
          ...
          const markerPromises = rentalLocations.map(
            async (location: BikeRentalLocation) => {
              if (!map.current) {
                return null;
              }
          ...
          bikeRentalMarkers.current = rentalMarkers.filter(
            (marker): marker is mapboxgl.Marker => marker !== null,
          );
    ...
    async (event: CustomEvent) => {
      ...
      if (coordinates) {
        ...
        const isBikeResource = bikeResources.some(
          (resource) =>
            resource.latitude === coordinates?.[1] &&
            resource.longitude === coordinates?.[0],
        );
        ...
        const isBikeRental = bikeRentalMarkers.current.some((marker) => {
          const markerLngLat = marker.getLngLat();
          return (
            markerLngLat.lng === coordinates?.[0] &&
            markerLngLat.lat === coordinates?.[1]
          );
        });
        ...
        if (showBikeRentals) {
          const rentalMarker = bikeRentalMarkers.current.find((marker) => {
            const markerLngLat = marker.getLngLat();
            return (
              markerLngLat.lng === coordinates?.[0] &&
              markerLngLat.lat === coordinates?.[1]
                ...
                if (route) {
                  ...
                  if (sourceId && sourceLayer) {
                    ...
                    if (features.length > 0) {
                      ...
                      if (
                        bounds.getNorth() !== undefined &&
                        bounds.getSouth() !== undefined
                      ) {
